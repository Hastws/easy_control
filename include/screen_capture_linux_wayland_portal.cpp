...
bool portal_screenshot_png(std::vector<unsigned char>& out_png) {
  ...
  GDBusProxy* proxy = g_dbus_proxy_new_for_bus_sync(
      ...
      nullptr, &err);
  if (!proxy) { if (err) g_error_free(err); return false; }
  ...
  GVariant* res = g_dbus_proxy_call_sync(proxy, "Screenshot",
      g_variant_new("(ssa{sv})", "", "", &opts),
      G_DBUS_CALL_FLAGS_NONE, -1, nullptr, &err);
  if (!res) { if (err) g_error_free(err); g_object_unref(proxy); return false; }

  const char* handle = nullptr;
  g_variant_get(res, "(o)", &handle);
  std::string handle_path = handle ? handle : "";
  g_variant_unref(res);
  if (handle_path.empty()) { g_object_unref(proxy); return false; }

  GDBusProxy* req = g_dbus_proxy_new_for_bus_sync(
      G_BUS_TYPE_SESSION,
      ...
      nullptr, &err);
  if (!req) { if (err) g_error_free(err); g_object_unref(proxy); return false; }

  bool got = false; std::string uri;
  auto on_signal = [](GDBusProxy* /*proxy*/, gchar* /*sender*/, gchar* signal,
                      GVariant* parameters, void* user_data) {
    if (std::string(signal) != "Response") return;
    bool* gotp = static_cast<bool*>(user_data);
    guint32 resp; GVariant* dict;
  ...
  void* udata[2]; udata[0] = &got; udata[1] = &uri;
  ...
  if (!got || uri.empty()) { g_object_unref(req); g_object_unref(proxy); return false; }

  GFile* file = g_file_new_for_uri(uri.c_str());
  if (!file) { g_object_unref(req); g_object_unref(proxy); return false; }

  GError* err2=nullptr;
  GFileInputStream* is = g_file_read(file, nullptr, &err2);
  if (!is) { if (err2) g_error_free(err2); g_object_unref(file); g_object_unref(req); g_object_unref(proxy); return false; }

  const gsize chunk = 64*1024;
  std::vector<unsigned char> buf; buf.reserve(256*1024);
  while (true) {
    unsigned char tmp[chunk];
    gssize n = g_input_stream_read(G_INPUT_STREAM(is), tmp, chunk, nullptr, &err2);
    if (n < 0) { if (err2) g_error_free(err2); g_object_unref(is); g_object_unref(file); g_object_unref(req); g_object_unref(proxy); return false; }
    if (n == 0) break;
...
std::string ScreenCapture::GetDisplayInfo(int index) {
  (void)index; return "Linux Wayland (xdg-desktop-portal)";
}
