...
namespace autoalg {
    ...
    public:
        ...
        inline Input() {
            ...
            if (wl_display_) {
                ...
                wl_registry_add_listener(
                    wl_registry_, &RegistryListener(), this);
        ...
        inline void MouseMoveTo(int x, int y) {
            ...
            in.mi.dx =
                    static_cast<LONG>((x * 65535LL) / std::max<std::size_t>(1, display_x_ - 1));
            in.mi.dy =
                    static_cast<LONG>((y * 65535LL) / std::max<std::size_t>(1, display_y_ - 1));
            ...
            zwlr_virtual_pointer_v1_motion_absolute(vp_dev_, 0, wl_fixed_from_double(
                                                        static_cast<double>(x) / std::max<double>(
                                                            1.0, static_cast<double>(display_x_))),
                                                    wl_fixed_from_double(
                                                        static_cast<double>(y) / std::max<double>(
                                                            1.0, static_cast<double>(display_y_))));
            zwlr_virtual_pointer_v1_frame(vp_dev_);
#else
            // Fallback: relative motion from last pos.
            zwlr_virtual_pointer_v1_motion(vp_dev_, 0,
                                           wl_fixed_from_double(x - cur_x_),
                                           wl_fixed_from_double(y - cur_y_));
            zwlr_virtual_pointer_v1_frame(vp_dev_);
#endif
            wl_display_flush(wl_display_);
            }
...
#ifdef __APPLE__
            CGEventRef e = CGEventCreateMouseEvent(nullptr, DownEvent_(button),
                                                   CGPointMake(cur_x_, cur_y_),
                                                   ToMouseButton_(button));
            CGEventPost(kCGHIDEventTap, e);
            CFRelease(e);
#elif defined(_WIN32)
            INPUT in{};
...
#ifdef __APPLE__
            CGEventRef e = CGEventCreateMouseEvent(nullptr, UpEvent_(button),
                                                   CGPointMake(cur_x_, cur_y_),
                                                   ToMouseButton_(button));
            CGEventPost(kCGHIDEventTap, e);
            CFRelease(e);
        ...
        inline void MouseDragTo(int x, int y, int button) {
...
#elif defined(_WIN32)
            {
                INPUT in{};
                in.type = INPUT_MOUSE;
                in.mi.dwFlags = (button == kRight
                                     ? MOUSEEVENTF_RIGHTDOWN
                                     : (button == kMiddle ? MOUSEEVENTF_MIDDLEDOWN : MOUSEEVENTF_LEFTDOWN));
                SendInput(1, &in, sizeof(in));
            }
#elif defined(__linux__)
            ...
            if (vp_dev_) {
                zwlr_virtual_pointer_v1_button(
                    vp_dev_, 0, LinuxBtnCode_(button), WL_POINTER_BUTTON_STATE_PRESSED);
                zwlr_virtual_pointer_v1_frame(vp_dev_);
                wl_display_flush(wl_display_);
            }
#elif defined(INPUT_BACKEND_UINPUT)
...
#elif defined(_WIN32)
            {
                ...
                in.mi.dwFlags = (button == kRight
                                     ? MOUSEEVENTF_RIGHTUP
                                     : (button == kMiddle ? MOUSEEVENTF_MIDDLEUP : MOUSEEVENTF_LEFTUP));
                SendInput(1, &in, sizeof(in));
            }
#elif defined(__linux__)
#if defined(INPUT_BACKEND_WAYLAND_WLR)
            if (vp_dev_) {
                zwlr_virtual_pointer_v1_button(
                    vp_dev_, 0, LinuxBtnCode_(button), WL_POINTER_BUTTON_STATE_RELEASED);
                zwlr_virtual_pointer_v1_frame(vp_dev_);
                wl_display_flush(wl_display_);
            }
#elif defined(INPUT_BACKEND_UINPUT)
        ...
        inline void ScrollLines(int dx, int dy) {
            ...
            if (vp_dev_) {
                ...
                if (dy) {
                    zwlr_virtual_pointer_v1_axis(vp_dev_, 0, WL_POINTER_AXIS_VERTICAL_SCROLL,
            ...
            if (dpy_) {
                ...
                if (dy > 0) emit(4, dy);
                else if (dy < 0) emit(5, -dy);
                if (dx > 0) emit(6, dx);
                else if (dx < 0) emit(7, -dx);
                XFlush(dpy_);
            }
#endif
#endif
...
#ifdef __APPLE__
            CGEventRef ev =
                    CGEventCreateKeyboardEvent(nullptr, static_cast<CGKeyCode>(key), true);
...
#ifdef __APPLE__
            CGEventRef ev =
                    CGEventCreateKeyboardEvent(nullptr, static_cast<CGKeyCode>(key), false);
            CGEventPost(kCGAnnotatedSessionEventTap, ev);
            CFRelease(ev);
#elif defined(_WIN32)
            INPUT in{};
        ...
        inline void KeyboardDownWithMods(int key, uint64_t mods) {
            ...
            if (vkb_dev_) {
                if (mods & kShift)
                    zwp_virtual_keyboard_v1_key(vkb_dev_, 0, LinuxKeyShift_(), WL_KEYBOARD_KEY_STATE_PRESSED);
                if (mods & kControl)
                    zwp_virtual_keyboard_v1_key(vkb_dev_, 0, LinuxKeyCtrl_(), WL_KEYBOARD_KEY_STATE_PRESSED);
                if (mods & kOption)
                    zwp_virtual_keyboard_v1_key(vkb_dev_, 0, LinuxKeyAlt_(),
                                                WL_KEYBOARD_KEY_STATE_PRESSED);
                if (mods & kCommand)
                    zwp_virtual_keyboard_v1_key(vkb_dev_, 0, LinuxKeySuper_(),
                                                WL_KEYBOARD_KEY_STATE_PRESSED);
        ...
        inline void KeyboardUpWithMods(int key, uint64_t mods) {
            ...
            if (vkb_dev_) {
                ...
                if (mods & kOption)
                    zwp_virtual_keyboard_v1_key(vkb_dev_, 0, LinuxKeyAlt_(), WL_KEYBOARD_KEY_STATE_RELEASED);
                if (mods & kControl)
                    zwp_virtual_keyboard_v1_key(vkb_dev_, 0, LinuxKeyCtrl_(),
                                                WL_KEYBOARD_KEY_STATE_RELEASED);
                if (mods & kShift)
                    zwp_virtual_keyboard_v1_key(vkb_dev_, 0, LinuxKeyShift_(),
                                                WL_KEYBOARD_KEY_STATE_RELEASED);
                wl_display_flush(wl_display_);
            }
#elif defined(INPUT_BACKEND_UINPUT)
        ...
        inline void TypeUTF8(const std::string &utf8_text) {
            ...
            CFStringRef cf = CFStringCreateWithBytes(kCFAllocatorDefault,
                                                     reinterpret_cast<const UInt8 *>(utf8_text.data()),
            ...
            for (size_t i = 0; i < utf8_text.size();) {
                ...
                if (c < 0x80) {
                    ...
                    cp = ((c & 0x0F) << 12) |
                         ((static_cast<unsigned char>(utf8_text[i + 1]) & 0x3F) << 6) |
                         (static_cast<unsigned char>(utf8_text[i + 2]) & 0x3F);
                    i += 3;
                } else {
                    cp = ((c & 0x07) << 18) |
                         ((static_cast<unsigned char>(utf8_text[i + 1]) & 0x3F) << 12) |
        ...
        inline int CharToKeyCode(char key_char) {
            ...
            if (!dict) {
                dict = CFDictionaryCreateMutable(kCFAllocatorDefault, 128,
                                                 &kCFCopyStringDictionaryKeyCallBacks, nullptr);
                for (std::size_t i = 0; i < 128; ++i) {
                    ...
                    UniChar chars[4] = {0};
                    UniCharCount real = 0;
                    if (UCKeyTranslate(layout, static_cast<CGKeyCode>(i), kUCKeyActionDisplay, 0,
                        ...
                        if (s) {
                            CFNumberRef num =
                                    CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &i);
            ...
            if (key) {
                ...
                if (CFDictionaryGetValueIfPresent(dict, key,
    ...
    private:
        ...
        inline void EmitDragPath_(int start_x, int start_y, int end_x, int end_y, int button) {
            ...
            for (int i = 1; i <= steps; ++i) {
                ...
                in.type = INPUT_MOUSE;
                in.mi.dwFlags = MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE;
                in.mi.dx = static_cast<LONG>((ix * 65535ll) /
                                             std::max<std::size_t>(1, display_x_ - 1));
                in.mi.dy = static_cast<LONG>((iy * 65535ll) /
                                             std::max<std::size_t>(1, display_y_ - 1));
                SendInput(1, &in, sizeof(in));
...
#ifdef ZWLR_VIRTUAL_POINTER_V1_MOTION_ABSOLUTE_SINCE_VERSION
                // 归一化到 [0,1]
                double nx = static_cast<double>(ix) /
                            std::max<double>(1.0, static_cast<double>(display_x_));
                double ny = static_cast<double>(iy) /
                            std::max<double>(1.0, static_cast<double>(display_y_));
                zwlr_virtual_pointer_v1_motion_absolute(
                    vp_dev_, 0, wl_fixed_from_double(nx), wl_fixed_from_double(ny));
                zwlr_virtual_pointer_v1_frame(vp_dev_);
                wl_display_flush(wl_display_);
#else
                zwlr_virtual_pointer_v1_motion(
                    vp_dev_, 0,
                    wl_fixed_from_double(ix - cur_x_),
                    wl_fixed_from_double(iy - cur_y_));
                zwlr_virtual_pointer_v1_frame(vp_dev_);
                wl_display_flush(wl_display_);
#endif
    }
        ...
        int screen_{0};
        inline int XButtonFromGeneric_(int b) {
            return b == kRight ? 3 : (b == kMiddle ? 2 : 1);
        }
        ...
        inline int LinuxBtnCode_(int b) const {
        ...
        static void RegistryGlobal_(void *data, wl_registry *reg, uint32_t name,
            ...
            if (strcmp(interface, wl_seat_interface.name) == 0) {
                ...
                self->vp_mgr_ =
                        static_cast<zwlr_virtual_pointer_manager_v1 *>(
