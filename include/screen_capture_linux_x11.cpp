...
inline int mask_shift(unsigned long mask){ int s=0; while(mask && !(mask&1)){ mask>>=1; ++s;} return s; }
inline uint8_t extract_chan(unsigned long pixel, unsigned long mask){
  if (!mask) return 0;
  int s = mask_shift(mask);
  unsigned long v = (pixel & mask) >> s;
  int bits=0; unsigned long m = mask>>s; while(m){++bits; m>>=1;}
...
bool ScreenCapture::CaptureScreenWithCursor(int displayIndex, ImageRGBA& out) {
  ...
  if (displayIndex<0 || displayIndex>=(int)mons.size()) { XCloseDisplay(dpy); return false; }
  ...
  if (cur) {
    ...
    for (int j=0;j<(int)cur->height;++j){
      int py = cy + j; if (py<0 || py>=m.h) continue;
      for (int i=0;i<(int)cur->width;++i){
        int px = cx + i; if (px<0 || px>=m.w) continue;
        uint32_t argb = cur->pixels[j*cur->width + i];
        uint8_t a = (argb>>24)&0xFF; if (!a) continue;
        uint8_t src[4] = {
...
std::string ScreenCapture::GetDisplayInfo(int index) {
  return "Linux X11 Monitor " + std::to_string(index);
}
